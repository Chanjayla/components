...
SliderObj.prototype = {
    ...
    createPagation: function() {
        ...
        for(var i=0;i<this.config.imgNum;++i) {
            if(i!==this.position) {
                htmlStr += '<span id="page_dot_'+i+'" ontouchend="globalApi.sliderObj.switchPage('+i+')"></span>';
            } else {
                htmlStr += '<span id="page_dot_'+i+'" ontouchend="globalApi.sliderObj.switchPage('+i+')" class="page-active"></span>';
            }
        }
    ...
    addMouseEvent: function() {
        var that = this;
        document.getElementById(this.config.eventPanel).onmousedown = function(event) {
            that.mousePos.upX = 0;
            that.mousePos.downX = event.clientX;
        };
        document.getElementById(this.config.eventPanel).onmouseup = function(event) {
            that.mousePos.upX = event.clientX;
            if (that.mousePos.upX - that.mousePos.downX > 30) {
                that.switch(-1);
            } else if (that.mousePos.upX - that.mousePos.downX < -30) {
                that.switch(1);
            }
        };
    }
